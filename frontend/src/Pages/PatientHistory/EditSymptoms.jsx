...
function EditSymptoms({ modalEdit, setModalEdit, patientHistorySymptoms, setPatientHistorySymptoms, editingSymptom, setEditingSymptom }) {
    ...
    // Popola i campi quando il modal viene aperto
    useEffect(() => {
        if (editingSymptom) {
            setType(editingSymptom.type || '');
            setCharacteristic(editingSymptom.characteristic || '');
            setPattern(editingSymptom.pattern || '');
            setIntensityNprs(editingSymptom.intensityNprs || 1);
            setFrequency(editingSymptom.frequency || '');
            setBetterWhen(editingSymptom.betterWhen || '');
            setWorseWhen(editingSymptom.worseWhen || '');
            setTrend(editingSymptom.trend || '');
        }
    }, [editingSymptom]);
    ...
    const handleSave = () => {
        ...
        const exists = patientHistorySymptoms.some(
            (s, i) => s.type === type && i !== editingSymptom.index
        );
        
        if (exists) {
            alert(`Esiste già un sintomo con il tipo ${type}. Deve essere univoco.`);
            return;
        }
        ...
        setPatientHistorySymptoms(prev =>
            prev.map((s, i) => i === editingSymptom.index ? updatedSymptom : s)
        );
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
            <div className="bg-white p-6 rounded shadow-lg relative w-1/2">
                <h2 className="text-xl font-bold mb-4">Modifica sintomo</h2>
                <form>
                    ...
                    <div className="w-full flex gap-3 p-1">
                        <div className="flex-1">
                            <label className="block mb-1">Costante; Intermittente (on-off?)</label>
                            <textarea
                                name="pattern"
                                rows={2}
                                className="w-full border p-2 rounded resize-none"
                    ...
                    <div className="w-full flex gap-3 p-1">
                        <div className="flex-1">
                            <label className="block mb-1">Frequenza (tempo/attività)</label>
                            <textarea
                                name="frequency"
                                rows={2}
                                className="w-full border p-2 rounded resize-none"
