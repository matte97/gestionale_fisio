...
import { PatientFilters } from "../Services/patients.service";
...
export function PatientListPage() {
  ...
  const { data: patientsData, isLoading, isFetching, error } = usePatientsList(page, filters);
  ...
  if (isLoading && !patientsData) return <div className="p-8 text-center text-gray-500 font-medium">Caricamento pazienti...</div>;
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col gap-6 shrink-0">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            {isFetching && (
              <div className="animate-spin h-5 w-5 border-2 border-indigo-500 border-t-transparent rounded-full ml-2"></div>
            )}
          </div>
